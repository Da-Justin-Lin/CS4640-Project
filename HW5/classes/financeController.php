...
class FinanceController {
    ...
    private function history() {
        $list = $this->db->query("select trans_name, trans_cat, t_date, amount, trans_type from hw5_transaction where user_id = ? order by t_date desc;");
        $total_bal = $this->db->query("select sum(amount) as balance from hw5_transaction where user_id = ?;");
        $cat_bal = $this->db->query("select trans_cat, sum(amount) where user_id = ? as balance from hw5_transaction group by category;");
    ...
    private function new() {
        if (isset($_POST["trans_type"])) {
            ...
            if ($_POST["trans_type"] == "credit") {
                $insert = $this->db->query("insert into hw5_transaction (name, trans_name, trans_cat, date, amount, trans_type, id) values (?, ?, ?, ?, ?, ?, ?);", 
                "ssssss", $_COOKIE["name"], $_POST["trans_name"], $_POST["trans_cat"], 
